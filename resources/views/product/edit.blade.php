                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                            <li class="active"><a href="">Product / </a>Edit</li> 
                        </ol>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col-md-4 offset-4">
                            <form action="{{ url('product/'.$product->id) }}" method="post">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" value="{{ old('nama', $product->nama)  }}" autofocus>
                                    @error('nama') 
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="brand_id" id="" class="form-control @error('brand_id') is-invalid @enderror" > 
                                        ...
                                        @foreach ($products as $item)
                                            <option value="{{ $item->id }}" {{ old('brand_id', $product->brand_id) == $item->id ? 'selected' : null }}>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="info" class="form-control @error('info') is-invalid @enderror">{{ old('info', $product->info) }}</textarea>
                                    @error('info')
                                        <div id="validationServer03Feedback" class="invalid-feedback">
                                            {{ $message }}
                                        </div>
